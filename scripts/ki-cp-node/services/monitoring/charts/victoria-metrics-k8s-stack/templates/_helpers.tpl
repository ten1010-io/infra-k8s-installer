...
{{- /*
Normalize image map for VM Operator CRs.
Some operator versions ignore `image.registry`, so compose it into `image.repository`.
*/ -}}
{{- define "vm.operator.image.normalize" -}}
  {{- $image := deepCopy (. | default dict) -}}
  {{- if and ($image.registry | default "") ($image.repository | default "") -}}
    {{- $prefix := printf "%s/" $image.registry -}}
    {{- if not (hasPrefix $prefix $image.repository) -}}
      {{- $_ := set $image "repository" (printf "%s/%s" $image.registry $image.repository) -}}
    {{- end -}}
  {{- end -}}
...
{{- define "vm.alert.spec" -}}
  ...
  {{- if $output.image -}}
    {{- $_ := set $output "image" (include "vm.operator.image.normalize" $output.image | fromYaml) -}}
  {{- end -}}
...
{{- define "vm.agent.spec" -}}
  ...
  {{- $output := mergeOverwrite (deepCopy $spec) (deepCopy $Values.vmagent.spec) -}}
  {{- if $output.image -}}
    {{- $_ := set $output "image" (include "vm.operator.image.normalize" $output.image | fromYaml) -}}
  {{- end -}}
  {{- tpl ($output | toYaml) . -}}
...
{{- define "vm.auth.spec" -}}
  ...
  {{- $_ := set $spec "image" (include "vm.operator.image.normalize" $spec.image | fromYaml) -}}
  {{- with (include "vm.license.global" .) -}}
    {{- $_ := set $spec "license" (fromYaml .) -}}
  {{- end -}}
  {{- tpl (toYaml $spec) . -}}
