...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --force                   OIDC가 이미 설정되어 있어도 강제 업데이트"
            ...
            echo ""
            echo "설명:"
            echo "  Keycloak을 사용한 Kubernetes API Server OIDC 인증을 설정하고"
            ...
            echo "    5. API Server에 OIDC 설정 파라미터 추가"
            echo "    6. API Server 자동 재시작 트리거"
            echo ""
            echo "로컬 실행:"
            echo "  ./5_oidc_and_ca.sh --config config.json"
            echo ""
            echo "원격 실행:"
...
check_ssh_connection() {
    ...
    if ssh $ssh_opts ${REMOTE_USER}@${REMOTE_HOST} "echo 'SSH connection successful'" &> /dev/null; then
...
execute_remote() {
    log_step "원격 호스트에서 OIDC 및 CA 설정 실행 중..."
    ...
    # 원격 명령 구성
    local remote_cmd="cd ${remote_script_dir} && bash ${script_name} --config $(basename ${CONFIG_FILE})"
...
check_oidc_configured() {
    log_step "기존 OIDC 설정 확인 중..."

    if grep -q "oidc-issuer-url" "$KUBE_APISERVER_MANIFEST"; then
        log_warn "kube-apiserver.yaml에 OIDC 설정이 이미 존재합니다"

        if [ "${FORCE_UPDATE:-false}" = true ]; then
            log_warn "강제 업데이트 활성화, OIDC를 재설정합니다"
            return 1
        else
            log_info "현재 OIDC 설정:"
            grep "oidc-" "$KUBE_APISERVER_MANIFEST" | sed 's/^/  /'
            echo ""
...
remove_existing_oidc_config() {
    ...
    # 모든 OIDC 관련 파라미터 제거
    ${YQ_COMMAND} -i 'del(.spec.containers[0].command[] | select(. == "*oidc*"))' "$KUBE_APISERVER_MANIFEST"
...
apply_oidc_config() {
    ...
    log_info "OIDC Issuer URL: $KEYCLOAK_ISSUER_URL"
    log_info "OIDC Client ID: k8s"
    log_info "OIDC CA File: /etc/kubernetes/pki/aipub-ca.crt"

    # OIDC 설정 추가
    ${YQ_COMMAND} -i ".spec.containers[0].command += [\"--oidc-issuer-url=${KEYCLOAK_ISSUER_URL}\"]" "$KUBE_APISERVER_MANIFEST"
    ${YQ_COMMAND} -i '.spec.containers[0].command += ["--oidc-client-id=k8s"]' "$KUBE_APISERVER_MANIFEST"
    ${YQ_COMMAND} -i '.spec.containers[0].command += ["--oidc-username-claim=preferred_username"]' "$KUBE_APISERVER_MANIFEST"
    ${YQ_COMMAND} -i '.spec.containers[0].command += ["--oidc-groups-claim=groups"]' "$KUBE_APISERVER_MANIFEST"
    ${YQ_COMMAND} -i '.spec.containers[0].command += ["--oidc-ca-file=/etc/kubernetes/pki/aipub-ca.crt"]' "$KUBE_APISERVER_MANIFEST"
    ${YQ_COMMAND} -i '.spec.containers[0].command += ["--oidc-username-prefix=oidc:"] | .spec.containers[0].command[-1] style="double"' "$KUBE_APISERVER_MANIFEST"
    ${YQ_COMMAND} -i '.spec.containers[0].command += ["--oidc-groups-prefix=oidc:"] | .spec.containers[0].command[-1] style="double"' "$KUBE_APISERVER_MANIFEST"
...
wait_for_apiserver() {
    ...
    log_info "  crictl ps | grep kube-apiserver"
    log_info "  crictl logs <container-id>"
    return 1
}
...
show_config() {
    ...
    echo "OIDC Issuer:     $KEYCLOAK_ISSUER_URL"
    echo "OIDC Client ID:  k8s"
    echo "Username Claim:  preferred_username"
    echo "Groups Claim:    groups"
    echo "CA File:         /etc/kubernetes/pki/aipub-ca.crt"
    echo "Username Prefix: oidc:"
    echo "Groups Prefix:   oidc:"
...
main() {
    ...
    if [ -z "$REMOTE_HOST" ]; then
        REMOTE_HOST=$(${JQ_COMMAND} -r '.control_plane.host // empty' "$CONFIG_FILE")
    fi
    if [ "$REMOTE_USER" = "root" ]; then
        REMOTE_USER=$(${JQ_COMMAND} -r '.control_plane.user // "root"' "$CONFIG_FILE")
    fi
    if [ "$REMOTE_PORT" = "22" ]; then
        REMOTE_PORT=$(${JQ_COMMAND} -r '.control_plane.port // "22"' "$CONFIG_FILE")
    fi
    ...
    if [ "${NODE_TYPE}" == "CP" ]; then
      ...
      if wait_for_apiserver; then
          ...
          echo "  3. Keycloak 사용자로 OIDC 인증 테스트"
          echo "  4. OIDC 사용자/그룹에 대한 RBAC 역할 설정"
          echo ""
          log_info "백업 위치: $BACKUP_FILE"
          log_info "복원 방법: cp $BACKUP_FILE $KUBE_APISERVER_MANIFEST"
          echo ""
      else
