...
IMAGE_TAG=$(${JQ_COMMAND} -r '.version.project_controller' "$CONFIG_FILE")
...
EXAMPLES:
    # Normal deployment
    ./project-controller.sh

    # Dry run to preview changes
    ./project-controller.sh --dry-run

    # Skip certificate generation
    ./project-controller.sh --skip-cert

    # Use custom image
    ./project-controller.sh --image ten1010io/project-controller:1.0.0

    # Skip multiple steps
    ./project-controller.sh --skip-cert --skip-labels
...
#for worker in $(kubectl get node | grep -Ev "control-plane|NAME" | awk '{ print $1 }'); do
#    echo "Processing worker node : $worker"
#    kubectl label nodes $worker \
#      project.aipub.ten1010.io/project-managed=true --overwrite
#done
#
## NodeGroup CR 적용
#echo "[Project Controller] NodeGroup 에 Worker 로드 추가"
#kubectl apply -f ../yaml/node-group.yaml
#for worker in $(kubectl get node | grep -Ev "control-plane|NAME" | awk '{ print $1 }'); do
#    echo "NodeGroup - Add worker node : $worker"
#    kubectl patch ng aipub-node-group --type='json' -p='[
#        {"op": "add", "path": "/spec/nodes/-", "value": "'"$worker"'"}
#    ]'
#done
#
## Project Controller Enabled 상태 변경
#kubectl patch cm -n project-controller project-controller-envs -p '{"data":{"APP_AIPUB_ENABLED":"false"}}'
#kubectl rollout restart deploy -n project-controller project-controller
