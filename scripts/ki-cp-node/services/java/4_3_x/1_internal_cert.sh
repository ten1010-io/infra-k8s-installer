...
create_ca() {
    ...
    cp "${crt_src}" "${K8S_PKI_DIR}/aipub-ca.crt"
    ...
    cp "${crt_src}" "${CA_OUTPUT_DIR}/ca.crt"
    log_info "Copying ca.key to output directory: ${key_src} -> ${CA_OUTPUT_DIR}/ca.key"
    cp "${key_src}" "${CA_OUTPUT_DIR}/ca.key"

    # Copy AIPub CA certificate to OS-specific trust directory
    if [ -n "$CA_TRUST_DIR" ]; then
        log_info "Copying CA to OS trust directory: ${crt_src} -> ${CA_TRUST_DIR}/aipub-ca.crt"
        mkdir -p "$CA_TRUST_DIR"
        cp "${crt_src}" "${CA_TRUST_DIR}/aipub-ca.crt"
