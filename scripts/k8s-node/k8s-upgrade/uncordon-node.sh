...
main() {
  ...
  upgrade_node=$($yq_cmd '.upgrade_node' < "$vars_path")
  ih_to_hostname_dict=$($yq_cmd -o json '.ih_to_hostname_dict' < "$vars_path")

  local node_knn
  node_knn=$(get_knn "$upgrade_node")

  msg "[INFO] Uncordoning node[\"${node_knn}\"]"
  kubectl uncordon "$node_knn"

  msg "[INFO] Node[\"${node_knn}\"] uncordoned"
