...
main() {
  ...
  k8s_upgrade_bin_path=$($yq_cmd '.k8s_upgrade_bin_path' < "$vars_path")
  k8s_binary_install_path=$($yq_cmd '.k8s_binary_install_path' < "$vars_path")

  msg "[INFO] Deploying k8s binaries (${k8s_upgrade_target_version}) -> ${k8s_binary_install_path}"

  require_directory_exists "$k8s_upgrade_bin_path"
  require_directory_exists "$k8s_binary_install_path"

  local binary
  while IFS= read -r binary; do
    local src="${k8s_upgrade_bin_path}/${binary}"
    local dst="${k8s_binary_install_path}/${binary}"
    require_file_exists "$src"
    msg "[INFO] Deploying: ${src} -> ${dst}"
    cp -f "$src" "$dst"
    chmod 0755 "$dst"
  done < <($yq_cmd '.k8s_upgrade_binaries[]' < "$vars_path")

  msg "[INFO] Deploy complete"
